...
def export_radiation_crax_config(config: cea.config.Configuration, output_folder: str, crax_result_folder: str, output_filename: str = "radiation_crax_config.json") -> Optional[Dict]:
    """
...
def main(config):
    ...
    zone_building_geometry_csv = generate_general_building_data_for_CRAX(zone_shapefile_path, input_folder, 'zone_building_geometry.csv')
    surroundings_building_geometry_csv = generate_general_building_data_for_CRAX(surroundings_shapefile_path, input_folder,
    ...
    use_latest_crax_binaries = content["use-latest-crax-binaries"]
    ...
    CRAX_bin_path, CRAX_lib_path = CRAXModel.check_crax_exe_directory(crax_bin_directory, use_latest_crax_binaries)
    ...
    if calculate_sensor_data:
        if using_cea_sensor:
            ...
            print("Grid generation of DAYSIM finished in %.2f mins" % ((time.time() - time1) / 60.0))
        else:
            time2 = time.time()
